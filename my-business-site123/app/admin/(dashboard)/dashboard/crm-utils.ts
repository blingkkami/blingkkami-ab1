...
export const getDDayColor = (deadline: string) => {
  if (!deadline) return 'text-zinc-500 bg-zinc-800 border-zinc-700';
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const target = new Date(deadline);
  target.setHours(0, 0, 0, 0);
  ...
  if (diffDays < 0) return 'text-pink-400 bg-pink-400/10 border-pink-400/20'; // Overdue
  if (diffDays <= 3) return 'text-rose-400 bg-rose-400/10 border-rose-400/20'; // Urgent
  if (diffDays <= 7) return 'text-purple-400 bg-purple-400/10 border-purple-400/20'; // Soon
  return 'text-zinc-300 bg-zinc-800 border-zinc-700'; // Plenty of time
};
